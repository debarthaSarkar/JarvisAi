...
DefaultMessage = f'''{Username}:Hello {Assistantname}, Hoe are you?
...
def ShowDefaultChatIfNoChats():
    try:
        # Open the ChatLog.json file in read mode
        with open(r'Data\ChatLog.json', "r", encoding='utf-8') as file:
            content = file.read()
            # Check if the file content is less than 5 characters (empty or nearly empty)
            if len(content) < 5:
                # Create or clear the Database.data file
                with open(TempDirectoryPath('Database.data'), 'w', encoding='utf-8') as db_file:
                    db_file.write("")
                # Write the default message to Responses.data
                with open(TempDirectoryPath('Responses.data'), 'w', encoding='utf-8') as resp_file:
                    resp_file.write(DefaultMessage)
    except FileNotFoundError:
        # If the ChatLog.json file doesn't exist, create it and retry
        print("ChatLog.json file not found. Creating a new one.")
        with open(r'Data\ChatLog.json', 'w', encoding='utf-8') as file:
            file.write("[]")  # Initialize with an empty JSON array
        ShowDefaultChatIfNoChats()  # Retry the function after creating the file
...
def FirstThread():
    while True:
        CurrentStatus = GetMicrophoneStatus("check_status")  # Pass the required argument
        if CurrentStatus == "True":
            MainExecution()
        else:
            AIStatus = GetAssistantStatus()
